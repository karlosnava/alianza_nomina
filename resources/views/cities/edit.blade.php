...
@section('content')
    ...
    <h1 class="font-bold text-2xl mb-5 mt-3">Editar ciudad {{ $city->name }} de {{ $country->name }}</h1>

    <form action="{{ route('cities.update', [$country, $city]) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="name" class="text-gray-700 font-semibold">Ciudad:</label>
            <input type="text" name="name" id="name" value="{{ old('name', $city->name) }}" class="w-full border rounded-md px-4 py-2" placeholder="Ej. Bogotá D.C" required autofocus>
            @error('name')
